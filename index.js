...
const CONFIG = {
  ...
  emailFrom: "M&A Radar <onboarding@resend.dev>",
...
async function fetchRumors() {
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 3000,
      system: `You are an M&A intelligence analyst. Search the web for the latest acquisition rumors from the past 24 hours. Focus on: ${CONFIG.searchTargets}.
...
- Include up to 6 rumors. Prioritize those with known stock tickers.`,
      ...
      messages: [
        {
          role: "user",
          content: `Search for M&A acquisition rumors, merger talks, takeover bids, and buyout speculation published in the last 24 hours for: ${CONFIG.searchTargets}. Include article URLs. Return JSON only.`,
        },
      ],
    }),
  });
...
// ─── Step 2: Fetch stock price with hard 8-second timeout ─────────────────
async function fetchStockPrice(ticker) {
  if (!ticker) return null;
  try {
    const url =
      "https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=" +
      ticker + "&apikey=" + CONFIG.alphaVantageKey;
...
function calcAcquisitionPrice(currentPrice, rumoredBidPrice, rumoredPremium) {
  ...
  if (rumoredBidPrice && rumoredBidPrice > currentPrice) {
    ...
    return { expectedPrice: rumoredBidPrice, premium: premium.toFixed(1), source: "rumored bid" };
  }
...
async function enrichRumors(rumors) {
  ...
  for (const r of rumors) {
    ...
    const priceCalc = stock ? calcAcquisitionPrice(stock.price, r.rumoredBidPrice, r.rumoredPremium) : null;
...
// ─── Step 6: Send email via Resend (HTTP — works on Railway) ───────────────
